...
class Webuzo_API extends Softaculous_API{	
	...
	function list_apps(){
		...
		$file = $this->curl_call('http://api.webuzo.com/apps.php?in=serialize');
		
		$this->apps = unserialize($file);
	...
	function list_installed_apps(){
		...
		if(empty($this->apps)){
			$this->error[] = 'Installed Apps were not loaded.';
			return false;
		}else{
			return true;	
		}
