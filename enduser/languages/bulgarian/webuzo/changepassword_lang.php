<?php

//////////////////////////////////////////////////////////////
//===========================================================
// changepassword_lang.php
//===========================================================
// SOFTACULOUS 
// Version : 1.1
// Inspired by the DESIRE to be the BEST OF ALL
// ----------------------------------------------------------
// Started by: Alons
// Date:       10th Jan 2009
// Time:       21:00 hrs
// Site:       http://www.softaculous.com/ (SOFTACULOUS)
// ----------------------------------------------------------
// Please Read the Terms of use at http://www.softaculous.com
// ----------------------------------------------------------
//===========================================================
// (c)Softaculous Inc.
//===========================================================
//////////////////////////////////////////////////////////////

if(!defined('SOFTACULOUS')){

	die('Hacking Attempt');

}

$l['no_new'] = 'Вие не сте уточнили новата парола.';
$l['no_conf'] = 'Липсва потвърждение на паролата.';
$l['no_match'] = 'Пароли не съвпадат';
$l['err_changing'] = 'Има грешка по време на смяна на паролата.';
$l['no_pass'] = 'You did not specify the password for File Manager';
$l['err_setting'] = 'There were some errors while setting the password for File Manager.';
$l['no_pass_tomcat'] = 'You did not specify the password for Apache Tomcat Manager.';
$l['err_setting_tomcat'] = 'There were some errors while setting the password for Apache Tomcat Manager.';

// Theme strings
$l['<title>'] = 'Промяна на паролата';
$l['submit_button'] = 'Промяна на паролата';
$l['new_pass'] = 'Нова парола';
$l['old_pass'] = 'Стара парола';
$l['retype_pass'] = 'Въведете пак новата парола';
$l['change_note'] = 'Промяна на паролата';
$l['change_final'] = 'Вашата парола е променена успешно';
$l['change_final_file'] = 'Your password for File Manager has been set successfully';
$l['change_final_tomcat'] = 'Your password for Apache Tomcat has been set successfully';
$l['change_onboot'] = 'Вашата парола ще бъде променена, когато VPS се зареди отново';
$l['filemanager'] = 'Set Password for File Manager';
$l['username'] = 'Username';
$l['fpassword'] = 'Password';
$l['tomcat'] = 'Set Password for Apache Tomcat';

?>