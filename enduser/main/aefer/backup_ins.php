...
if(!empty($data['backup_db']) && !empty($data['softdb'])){
	...
	$dbfile = $path.'/tmp/'.$name.'/softsql.sql';
	...
	$sql_conn = mysql_connect($data['softdbhost'], $dbuser, $dbpass);
...
if(!empty($data['backup_dir'])){
	...
	// File Permission
	$GLOBALS['bfh']['softperms'] = @fopen($data['path'].'/tmp/'.$data['name'].'/softperms.txt','w');
...
if(!empty($data['backup_datadir']) && !empty($data['softdatadir'])){
	...
	$GLOBALS['bfh']['datadir_softperms'] = fopen($data['path'].'/tmp/'.$data['name'].'/datadir_softperms.txt','w');
...
if(!empty($data['backup_wwwdir']) && !empty($data['wwwdir'])){
	...
	// We now create the file for storing file permission of datadir. We create here handler for the file.
	// Data will be written in class file of tar.
	$GLOBALS['bfh']['wwwdir_softperms'] = fopen($data['path'].'/tmp/'.$data['name'].'/wwwdir_softperms.txt','w');
