...
class softtar
{
    ...
    function softtar($p_tarname, $p_compress = null)
    {
...
global $globals, $theme, $softpanel, $iscripts, $catwise, $error;
...
if(!empty($data['restore_dir'])){
	...
	if(!empty($data['soft_version'])){
		if(!untar_archive($data['backup_dir'].'/'.$data['fname'], $data['softpath'])){
	...
	if(is_array($perms)){
		foreach($perms as $k => $v){
			...
			$perm = substr($v, -4);
			if(is_numeric($perm)){
				@chmod($data['softpath'].'/'.substr($v, 0, -5), octdec($perm));
			}
		}
	}
...
if(!empty($data['restore_db'])){
	...
	// Does the USER exist ?
	$mysql = @mysql_connect($data['softdbhost'], $dbuser, $dbpass, true);
	
	// Try to select the DB if connection was successful
	if($mysql){
		$_mysql = @mysql_select_db($data['softdb'], $mysql);
	}
	...
	if(!empty($data['soft_version'])){
		if(!file_exists($data['softpath'].'/'.$data['dbexist'])){
			if(!untar_archive($data['backup_dir'].'/'.$data['fname'], $data['softpath'], array($data['dbexist']))){
				$error[] = 'There was some error while unzipping the backup files';
				softdie('restoreerror');
			}
		}
	...
	//Make the Connection
	$__conn = @mysql_connect($data['softdbhost'], $dbuser, $dbpass, true);
...
'.mysql_error($__conn);
	...
	$res = mysql_query("SET foreign_key_checks = 0", $__conn);
	
	foreach($tables as $k => $v){
		$res = mysql_query("DROP TABLE `$v`", $__conn);
	}
