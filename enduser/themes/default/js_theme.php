	...
	if(!empty($installations[$soft])){
		...
		foreach($installations[$soft] as $ik => $iv){
			...
			$info['overview'] .= '<tr id="trid' . $ik . '">
				...
				<td>'.
				(!empty($is_cloneable) ? '<a href="'.$globals['ind'].'act=sclone&insid='.$ik.'" title="'.$l['clone'].'" class="someclass"><img src="'.$theme['images'].'clone.gif" /></a>&nbsp;' : '').
				'&nbsp;<a href="'.$globals['ind'].'act=remove&insid='.$ik.'" title="'.$l['remove'].'" class="someclass"><img src="'.$theme['images'].'remove.gif" /></a></td>
				<td>
