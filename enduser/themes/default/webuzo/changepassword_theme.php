...
echo '<center class="sai_tit"><img src="'.$theme['a_images'].'changepassword.gif" />&nbsp;'.$l['change_note'].'</center>';
echo '<script language="javascript" type="text/javascript"><!-- // --><![CDATA[

	var message_box = function(){			
			return {
				show_message: function(title, body , image) {			
					if(trim(body) == "'.$l['change_final'].'"){			
						var okbutton = \'<input  style="width:75px" class="sai_submit" type="button" onclick="message_box.close_message(1);" value="OK" />\';
					}else{
	...
	$(document).ready(function(){
			...
			// For changing password	
			$("#submitfilepass").click(function(){
				$("#createfilepass").css("display", "");					
				$.ajax({
					type: "POST",
					url: window.location+"&ajaxfilepass=1",					
					data: $("#editfileform").serialize(),
					
					// Checking for error
					success: function(data){
						$("#createfilepass").css("display", "none");
						var result = data.substring(0,1);
						if(result == "1"){
							$_("filepass").value="";							
							var msg = data.substring(1);
			...
			// For changing password	
			$("#submittomcatpass").click(function(){
				$("#createtomcatpass").css("display", "");					
				$.ajax({
					type: "POST",
					url: window.location+"&ajaxtompass=1",					
					data: $("#edittomcatform").serialize(),
					// Checking for error
					success: function(data){
						$("#createtomcatpass").css("display", "none");
						var result = data.substring(0,1);
						if(result == "1"){
							$_("tomcatpass").value="";							
							var msg = data.substring(1);
...
echo '<form accept-charset="'.$globals['charset'].'" action="" method="post" name="editfileform" id="editfileform">
	<center class="sai_tit"><img src="'.$theme['a_images'].'changepassword.gif" />&nbsp;'.$l['filemanager'].'</center><br />
	<table border="0" cellpadding="8" cellspacing="8" width="700px" align="center" class="sai_divroundshad"  style="padding-left:15px; padding-top:15px; ">
		<tr>
			<td width="40%">
				<span class="sai_head">'.$l['username'].'</span>
			</td>
			<td valign="top">
				<span class="sai_head">'.$user_name.'</span>
			</td>
		</tr>
		
		<tr>
			<td>
				<span class="sai_head">'.$l['fpassword'].'</span>
			</td>
			<td valign="top">
				<input type="password" name="filepass" id="filepass" size="30" value="" />
			</td>
		</tr>
		<tr>
			<td colspan="2">		
				<p align="center">
					<input type="hidden" name="changefilepass" value="1" />
					<input type="submit" style="cursor:pointer" value="'.$l['submit_button'].'" name="changefilepass" class="sai_graybluebut" id="submitfilepass"/> &nbsp;<img id="createfilepass" src="'.$theme['images'].'progress.gif" style="display:none">
				</p>
			</td>
		</tr>		
	</table>	
...
echo '<form accept-charset="'.$globals['charset'].'" action="" method="post" name="edittomcatform" id="edittomcatform">
	<center class="sai_tit"><img src="'.$theme['a_images'].'changepassword.gif" />&nbsp;'.$l['tomcat'].'</center><br />
	<table border="0" cellpadding="8" cellspacing="8" width="700px" align="center" class="sai_divroundshad" style="padding-left:15px; padding-top:15px; ">
		<tr>
			<td width="40%">
				<span class="sai_head">'.$l['username'].'</span>
		...
		<tr>
			...
			<td valign="top">
				<input type="password" name="tomcatpass" id="tomcatpass" size="30" value="" />
			</td>
		</tr>
		<tr>
			<td colspan="2">		
				<p align="center">
					<input type="hidden" name="changetomcatpass" value="1" />
					<input type="submit" style="cursor:pointer" value="'.$l['submit_button'].'" name="changetomcatpass" class="sai_graybluebut" id="submittomcatpass"/> &nbsp;<img id="createtomcatpass" src="'.$theme['images'].'progress.gif" style="display:none">
				</p>
			</td>
		</tr>		
	</table>	
