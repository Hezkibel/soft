...
echo '<br /><center class="sai_tit"><img src="'.$theme['a_images'].$mysql.'_icon.gif" />&nbsp;'.$iapps[$mysql.'_1']['name'].' '.$l['configuration'].'</center><br />';
...
echo '<script language="javascript" src="'.js_url('js/combined.js').'" type="text/javascript"></script>
	...
	echo '<form accept-charset="'.$globals['charset'].'" name="editemailsettings" method="post" action="" id="editmysql">
		<div style="height:400px; width:700px; overflow:auto; border: 2px solid #CCC; margin:0px auto; ">
			<table border="0" cellpadding="8" cellspacing="8" width="700px" align="center" class="sai_divroundshad">
				...
				echo '</td><td nowrap="nowrap"><textarea style=" overflow:hidden; font-size:12px; border:none; resize: none;" WRAP=OFF name="mysql_data" id="mysql_data" rows='.$linecount.' cols=120 >'.htmlentities(file_get_contents($filename), ENT_QUOTES, "UTF-8").'</textarea>
					</td>
				</tr>
			</table>
		</div><br />
		<p align="center">			
			<input type="submit" value="'.$l['save'].'" name="savemysql" class="sai_graybluebut" id="savemysql" /> &nbsp;<img id="save_apache" src="'.$theme['images'].	'progress.gif" style="display:none">
		</p>
	</form>';
