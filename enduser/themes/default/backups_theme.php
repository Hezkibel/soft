...
<div class="bg2">
	...
	<tr>
		<thead class="sai_head2" style="background:#333; color:#fff;">
			...
			(!empty($tmpbackup_info['ver']) ? '<th width="8%" align="left"><b>'.$l['version'].'</b></th>' : '').'
				...
				<tr>
					...
					foreach($backs as $bk => $bv){
						...
						foreach($bv as $ik => $iv){
							...
							(!empty($tmpbackup_info['ver']) ? '<td width="8%">'.$iv['ver'].'</td>' : '').
							(!empty($iv['backup_note']) ? '<td width="5%"><img src="'.$theme['images'].'note15.png" onmouseover="showtip(\''.$iv['backup_note'].'\', this);" style="cursor:pointer;"/></td>' : '<td width="4%">&nbsp</td>')
							.'
							<td width="4%">';
