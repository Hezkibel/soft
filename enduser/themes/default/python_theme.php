...
function get_package(){	
	if(nopackage){
		...
		try{		
			AJAX("'.$globals['index'].'act=python&soft='.$soft.'&ajaxdownload=1&random="+Math.random(), "get_package_handle(re)");
		}catch(e){ }
...
function checkhttps(proto_id, softdomain_id, alrt){
	try{
		...
		if(proto.indexOf("https") !== -1){
			 ...
			 $.ajax({
				type: "POST",
				url: "'.$globals['index'].'act=python&soft='.$soft.'&checkhttps="+encodeURIComponent(proto+$_(softdomain_id).value),
				timeout:10000,
				// Checking for error
				success: function(data){
...
// Advance option 
$info['install'] .= '
<br /><br /><table width="100%" cellpadding="8" cellspacing="1" border="0" class="sai_divroundshad">
	<tr class="nohover">
