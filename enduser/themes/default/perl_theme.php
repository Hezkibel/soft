	...
	if(!empty($installations[$soft])){
		...
		foreach($installations[$soft] as $ik => $iv){
			...
			$info['overview'] .= '<tr id="trid' . $ik . '">
				...
				<td>'.$iv['ver'].($software['ver'] != $iv['ver'] && $is_upgradable ? ' <a href="'.$globals['ind'].'act=perl_upgrade&insid='.$ik.'"><img src="'.$theme['images'].'updates.gif" alt="'.$l['upd_to'].' '.implode(' or ', $is_upgradable).'" title="'.$l['upd_to'].' '.implode(' or ', $is_upgradable).'" class="someclass"/></a>' : '').'</td><td>'.
